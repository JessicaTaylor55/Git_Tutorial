...
pkg <- c("knitr", "usethis", "gitcreds", "remotes")
pkg2 <-"klippy"
...
-   Use branches (copies) to work on documents in a repository without changing the master.
...
Load the following packages in RStudio. If you do not have them installed, you can do so using `install.packages()`, or run the `Chapter_12_packages.R` script in this repository. 
...
Before generating a new PAT, check to see if you already have one. 

```{r}
gitcreds::gitcreds_set()
```

The first line of code in the code chunk below, `create_github_token()`, will create a PAT for you, however, you do not need to run this if you already have one. If you are unsure if you have already input your PAT into RStudio, you can run `gitcreds_set()` and R will tell you your current credentials. Select option `3: See the password / token`. If your PAT is not shown here, select option `2: Replace these credentials` to change the current password to your new token.
